...
interface SearchInfo {
    stages: Array<'searching' | 'reading' | 'writing' | 'error'>;
    query: string;
    urls: string[];
    error?: string;
}
...
const PremiumTypingAnimation: React.FC = React.memo(() => {
    try {
        return (
        ...
        console.error('Error rendering typing animation:', error);
        return <div className="text-red-500 text-xs">Error displaying typing animation</div>;
    }
});
...
const SearchStages: React.FC<SearchStagesProps> = React.memo(({ searchInfo }: SearchStagesProps) => {
    try {
        ...
        console.error('Error rendering search stages:', error);
        return <div className="text-red-500 text-xs">Error displaying search information</div>;
    }
});
...
const MessageItem: React.FC<MessageItemProps> = React.memo(({ message }: MessageItemProps) => {
    return (
        <div className={`flex ${message.isUser ? 'justify-end' : 'justify-start'} mb-5`}>
...
const MessageArea: React.FC<MessageAreaProps> = React.memo(({ messages }: MessageAreaProps) => {
    // Function to generate stable keys for messages
    const getMessageKey = (message: Message) => `message-${message.id}`;
